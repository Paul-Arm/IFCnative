...
- [~] Repair suggestions/actions. First-pass suggestions exist for STEP envelope, missing refs, duplicate GlobalIds, containment, placement, and representation diagnostics; duplicate GlobalId diagnostics now expose a staged repair action that regenerates duplicate ids after the first occurrence, and multiple-primary-containment diagnostics can now stage a repair that keeps the first containment relationship while removing duplicate product links through the diff/export gate.
- [ ] IDS/MVD validation.
