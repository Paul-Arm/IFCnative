...
- [~] Safe targeted serializer preserving formatting/order where practical. Native STEP export now keeps parsed DATA entity order and preserves untouched entity source text/formatting, while edited/new entities serialize canonically.
...
- [~] Native relationship graph/editor with filters/depth/pinning. Relationship endpoints can now be selected and staged from the inspector, and a first-pass relationship neighborhood graph panel lists directional neighbors for the selected entity; richer filter/depth controls and visual graph layout remain pending.
...
- [~] Native validation panel with grouped severity. Diagnostics are now projected into severity-ranked rows with first-pass repair suggestions; richer filtering still pending.
- [~] Duplicate GlobalId, missing refs, physical product placement/representation, and multiple primary containment diagnostics.
- [~] Repair suggestions. First-pass suggestions exist for STEP envelope, missing refs, duplicate GlobalIds, containment, placement, and representation diagnostics.
- [ ] IDS/MVD validation.
...
4. [~] Split native shell into UI + services + view models so large lists can be virtualized and tested. Selection/inspector projection, navigation/type/search/bookmark projection, placement editor projection, entity-edit draft creation, and draft-session state have moved out of `MainWindow` into service/view-model classes; command wiring still needs further extraction.
5. [~] Port TypeScript native document capabilities to C# services: first-pass relationship model/indexing, property/resource/type/unit/placement/representation read indexes, numeric placement edit helper, and existing-product body assignment helper done; broader create/delete edit helpers remain.
