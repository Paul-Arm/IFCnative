...
- [~] Robust syntax recovery for malformed huge files. Native parser now skips unterminated entity argument rows and rows with missing entity types, preserves/re-serializes valid rows that are missing a terminating semicolon, strips unexpected trailing text between an argument list and semicolon, and skips duplicate STEP ids with warnings, preserving the first duplicate id and resuming parsing following valid STEP entities; broader malformed-input recovery remains pending.
