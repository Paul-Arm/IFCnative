...
```powershell
# local build
dotnet build IfcToGlb/IfcToGlb.Cli/IfcToGlb.Cli.csproj -c Release

# self-contained binaries (no .NET runtime required on target)
dotnet publish IfcToGlb/IfcToGlb.Cli/IfcToGlb.Cli.csproj -c Release -r linux-x64 --self-contained
dotnet publish IfcToGlb/IfcToGlb.Cli/IfcToGlb.Cli.csproj -c Release -r win-x64 --self-contained
```

Output lands in `IfcToGlb.Cli/bin/Release/net10.0/<rid>/publish/`. Notes:
